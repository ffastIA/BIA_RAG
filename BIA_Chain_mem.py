...
prompt_inicial = ChatPromptTemplate.from_messages([
    ("system", """Você é um copywriter especializado no projeto Biometria Inteligente para Aquicultura (BIA).
...
st.markdown("""
Esta aplicação permite que você analise o projeto BIA usando Inteligência Artificial **com memória conversacional**.
...
def create_rag_response_with_history(question: str, session_id: str) -> str:
    ...
    full_prompt = f"""Você é um copywriter especializado no projeto Biometria Inteligente para Aquicultura (BIA).
        Sua expertise é esclarecer dúvidas sobre este projeto de forma conversacional e personalizada.
...
def create_translation_with_history(text: str, session_id: str) -> str:
    ...
    translation_prompt_text = f"""Você é um tradutor especializado em textos técnicos sobre aquicultura e biometria.
    ...
    try:
        ...
        try:
            ...
            simple_response = chat_instance.invoke(
                f"Baseado no contexto sobre o projeto BIA, responda de forma amigável: {query}\n\nContexto: {context}"
            ).content
